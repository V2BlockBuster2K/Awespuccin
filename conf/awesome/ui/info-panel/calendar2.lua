...
local function date_widget(date, is_current, is_another_month)
	local text_color = beautiful.xcolorT1
	if is_current == true then
		text_color = beautiful.xcolor7
	elseif is_another_month == true then
		text_color = beautiful.xcolorO2
	end
	...
	return wibox.widget({
		...
		forced_height = dpi(30),
		shape = gshape.circle,
		bg = is_current and beautiful.xcolorS0,
		widgets.text({
			halign = "center",
			size = 10,
			color = text_color,
...
local function new()
	...
	ret.month = widgets.button.text.normal({
		...
		size = 16,
		bold = true,
		text_normal_bg = beautiful.xcolorT1,
		normal_bg = beautiful.xcolorO2,
		on_release = function()
